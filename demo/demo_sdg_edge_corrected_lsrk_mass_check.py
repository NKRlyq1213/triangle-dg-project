...
from operators.sdg_edge_streamfunction_corrected_rhs import (
    build_corrected_operator,
    edge_streamfunction_closed_sphere_rhs_corrected,
)
from operators.sdg_edge_streamfunction_closed_sphere_rhs import edge_flux_pair_summary
...
def run_lsrk(q0: np.ndarray, op, *, dt: float, steps: int):
    ...
    max_abs_cell_correction = 0.0
    max_local_mass_error_before = 0.0
    max_local_mass_error_after = 0.0

    for _ in range(steps):
        for a, b in zip(RK4A, RK4B):
            rhs, info = edge_streamfunction_closed_sphere_rhs_corrected(
                q,
                op,
                return_info=True,
            )
            ...
            max_abs_cell_correction = max(
                max_abs_cell_correction,
                abs(info["max_abs_cell_correction"]),
            )
            max_local_mass_error_before = max(
                max_local_mass_error_before,
                abs(info["max_abs_local_mass_error_before"]),
            )
            max_local_mass_error_after = max(
                max_local_mass_error_after,
                abs(info["max_abs_local_mass_error_after"]),
            )
...
def run_one_case(
    *,
    ...
    rhs0, info0 = edge_streamfunction_closed_sphere_rhs_corrected(
        q0,
        op,
        return_info=True,
    )
    ...
    return {
        ...
        "rhs0_max_abs_cell_correction": float(info0["max_abs_cell_correction"]),
        "rhs0_local_mass_error_before": float(info0["max_abs_local_mass_error_before"]),
        "rhs0_local_mass_error_after": float(info0["max_abs_local_mass_error_after"]),

        "max_abs_cell_correction": float(max_abs_cell_correction),
        "max_local_mass_error_before": float(max_local_mass_error_before),
        "max_local_mass_error_after": float(max_local_mass_error_after),
...
def print_table(rows: list[dict]) -> None:
    ...
    for r in rows:
        print(
            ...
            f"{r['max_abs_cell_correction']:12.4e} "
            f"{r['max_local_mass_error_after']:12.4e} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Corrected edge-streamfunction closed-sphere LSRK mass check."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(ROOT / "outputs" / "sdg_edge_corrected_lsrk_mass"),
    )
    ...
    print("=== SDG corrected edge-streamfunction LSRK mass check ===")
    ...
    csv_path = output_dir / f"edge_corrected_lsrk_mass_{args.q_case}_tau{args.tau:g}.csv"
