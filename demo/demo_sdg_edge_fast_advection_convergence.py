...
def rotate_points_by_axis_angle(
    X: np.ndarray,
    Y: np.ndarray,
    Z: np.ndarray,
    *,
    ...
    """
    Rodrigues rotation for arrays of points.

    Returns R_axis(angle) * X.
    """
...
def backward_characteristic_points(op, t: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    For dX/dt = Omega x X:

        X(t) = R_{Omega}(t) X(0)

    Therefore exact advection solution is:

        q(X,t) = q0(R_{Omega}(-t) X).
    """
    base = op.base
...
def spherical_gaussian_from_points(
    X: np.ndarray,
    Y: np.ndarray,
    Z: np.ndarray,
    *,
...
def exact_state(case: str, op, t: float, *, sigma: float = 0.35) -> np.ndarray:
    """
    Exact solution q(X,t) = q0(R(-t)X).

    Supported cases:
        constant, sphere_x, sphere_y, sphere_z, gaussian
    """
    ...
    raise ValueError("exact_state supports: constant, sphere_x, sphere_y, sphere_z, gaussian.")
...
def error_stats(q_num: np.ndarray, q_ex: np.ndarray, op) -> tuple[float, float, float]:
    err = q_num - q_ex
...
def run_one_case(
    *,
    ...
    # Warm up / compile fast RHS outside timing.
...
def add_rates(rows: list[dict]) -> None:
    """
    Add rates within each alpha group, sorted by nsub.
    """
    groups = {}
    for r in rows:
        key = (r["alpha0"], r["q_case"])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fast edge-streamfunction advection exact-solution convergence test."
    )
    ...
    parser.add_argument("--steps-per-nsub", type=int, default=20)
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(ROOT / "outputs" / "sdg_edge_fast_advection_convergence"),
    )
    ...
    print("=== SDG edge-streamfunction fast advection convergence ===")
    ...
    print(f"steps_per_nsub = {args.steps_per_nsub}")
    ...
    for alpha0 in alphas:
        for nsub in args.nsubs:
            steps = int(args.steps) if args.steps is not None else int(args.steps_per_nsub * nsub)
    ...
    csv_path = output_dir / (
        f"edge_fast_advection_convergence_{args.q_case}_T{args.final_time:g}_tau{args.tau:g}.csv"
    )
