...
from operators.sdg_streamfunction_closed_sphere_rhs import (
    ...
    closed_sphere_streamfunction_rhs,
    mass,
    rhs_mass_residual,
    weighted_l2_linf,
    q_initial,
...
def run_lsrk(
    ...
    *,
    ...
    """
    Five-stage low-storage RK.

    Returns:
        q, max_stage_mass_rhs_abs, max_stage_rhs_l2
    """
    ...
    for _step in range(steps):
        for a, b in zip(RK4A, RK4B):
            rhs = closed_sphere_streamfunction_rhs(q, op)

            m_rhs = rhs_mass_residual(rhs, op)
            rhs_l2, _rhs_linf = weighted_l2_linf(rhs, op)
    ...
    return q, max_stage_mass_rhs_abs, max_stage_rhs_l2
...
def run_one_case(
    *,
    ...
    q0 = q_initial(q_case, op)

    rhs0 = closed_sphere_streamfunction_rhs(q0, op)
    rhs0_mass = rhs_mass_residual(rhs0, op)
    rhs0_l2, rhs0_linf = weighted_l2_linf(rhs0, op)
    ...
    qT, max_stage_mass_rhs_abs, max_stage_rhs_l2 = run_lsrk(
        q0,
        op,
        dt=dt_eff,
        steps=steps,
    ...
    qT_l2, qT_linf = weighted_l2_linf(qT, op)

    seam = seam_summary(op)
    pair = flux_pair_summary(q0, op)
    ...
    return {
        ...
        "pairErr_initial": float(pair["max_pair_error"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Closed-sphere SDG stream-function RHS + LSRK mass drift check."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(ROOT / "outputs" / "sdg_closed_sphere_lsrk_mass"),
    )
    ...
    print("=== SDG closed-sphere LSRK mass check ===")
    ...
    csv_path = output_dir / f"closed_sphere_lsrk_mass_{args.q_case}_tau{args.tau:g}.csv"
