...
from operators.sdg_edge_streamfunction_global_corrected_rhs import (
    build_global_corrected_operator,
...
def run_lsrk(q0, op, *, dt: float, steps: int):
    ...
    for _ in range(steps):
        for a, b in zip(RK4A, RK4B):
            rhs, info = edge_streamfunction_closed_sphere_rhs_global_corrected(
                q,
                op,
                return_info=True,
            )
    ...
    return q, max_stage_mass_rhs_abs, max_stage_rhs_l2, max_stage_corr
...
def run_one_case(
    *,
    ...
    op = build_global_corrected_operator(
        nsub=nsub,
        order=order,
        N=N,
        R=R,
    ...
    rhs0, info0 = edge_streamfunction_closed_sphere_rhs_global_corrected(
        q0,
        op,
        return_info=True,
    )
    ...
    qT, max_stage_mass_rhs_abs, max_stage_rhs_l2, max_stage_corr = run_lsrk(
        q0,
        op,
        dt=dt_eff,
        steps=steps,
    ...
    # Constant-state deviation diagnostic
    if q_case == "constant":
    ...
    return {
        ...
        "rhs0_correction_constant": float(info0["global_correction_constant"]),
        ...
        "relative_mass_drift": float(abs(drift) / max(1.0, abs(m0))),
...
def main():
    parser = argparse.ArgumentParser(
        description="Global-corrected edge-streamfunction LSRK mass check."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(ROOT / "outputs" / "sdg_edge_global_corrected_lsrk_mass"),
    )
    ...
    print("=== SDG global-corrected edge-streamfunction LSRK mass check ===")
    ...
    csv_path = out / f"edge_global_corrected_lsrk_mass_{args.q_case}_tau{args.tau:g}.csv"
