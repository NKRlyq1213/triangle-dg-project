...
from visualization.sdg_mapping_diagnostics import write_all_sdg_mapping_diagnostics


def parse_args():
    p = argparse.ArgumentParser(description="SDG explicit T1--T8 mapping diagnostics.")
...
def main():
    ...
    outdir = (
        ...
        else ROOT / "outputs" / "sdg_mapping" / f"order{args.order}_nsub{args.n_sub}_R{args.R:g}"
    )

    paths = write_all_sdg_mapping_diagnostics(
        VX=VX,
        VY=VY,
        EToV=EToV,
        elem_patch_id=elem_patch_id,
    ...
    print("\n=== SDG per-patch diagnostics ===")
    pp = per_patch_diagnostics(cache, R=args.R)
