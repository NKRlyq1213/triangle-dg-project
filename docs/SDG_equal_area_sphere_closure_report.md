# Equal-Area SDG on Sphere 結案報告

> 專題方向：在球面上建立同時處理極點 / patch corner 奇異性、等面積投影、質量守恆與 free-stream preservation 的 SDG 離散方案。  
> 報告版本：v0.1  
> 日期：2026-05-05
...
2. **closed-sphere global mass conservation**：
   \[
   \frac{d}{dt}\int_{S^2}q\,dS\approx0.
   \]

3. **sphere seam pairing**：
   使用 \((X,Y,Z)\) embedding 配對 seam，避免重新引入 \((\lambda,\theta)\) 或 \(A^{-1}\) 極點方向不唯一問題。

4. **short-time accuracy**：
   \(T=0.1\) Gaussian advection 可觀察到接近四階的 \(L^2\) 收斂。

5. **工程效能**：
   Numba fast RHS 與 reference RHS 數值一致，且 RHS benchmark 有約 \(40\times\sim120\times\) 加速。
...
目前 evidence 顯示這不是 CFL / time integration 問題，而是 spatial consistency、seam / patch corner regularity、或 volume-face flux mismatch 長時間累積所造成。
...
1. 需要使用 \(A\)、\(A^{-1}\) 進行球體與八面體 / 平面 patch 間的映射與反映射。
2. 在極點或 patch corner 附近，反映射可能需要從多個 face 取值，導致極限方向不唯一。
3. 原本以 \(A^{-1}\) 或 velocity transform 得到的速度場，在 \(\alpha_0=\pi/4\) 時，divergence diagnostic 不像 \(\alpha_0=0\) 那樣漂亮。
4. 仍然希望保留 **equal-area mapping**，避免面積權重複雜化或質量積分不自然。

### 1.2 本階段目標

本階段目標可拆成五個：

1. 消除 \(A^{-1}\) 在極點 / corner 的方向不唯一問題。
2. 保留 equal-area formulation。
3. 建立 divergence-free 或 free-stream-preserving 的 velocity / flux 表示。
4. 建立 closed sphere 上的 conservative DG surface coupling。
5. 進入可跑 convergence test 的 fast implementation。

---

## 2. 第一階段：放棄 \(A^{-1}\) velocity，改用 stream-function area flux
...
- `L2_stream` 對所有 \(\alpha_0\) 都接近 roundoff。
- 相較之下，`L2_Ainv` 在 \(\alpha_0\neq0\) 時約 \(10^{-1}\) 等級。
- 對 constant volume RHS：
  \[
  RHS\approx0.
  \]
- full RHS constant-state check 初步通過。
...
## 3. 第二階段：projected surface lifting
...
- constant field：surface projected lifting 不破壞常數。
- `sphere_z`：由於跨 element 連續，surface jump 幾乎為 0。
- `element_jump` / `element_checker`：surface penalty 被真正啟動，且 projected 與 diagonal lifting 明顯不同。
...
## 4. 第三階段：strong correction surface 與 direct numerical flux surface

### 4.1 初始 strong correction 問題
...
對 discontinuous field，global mass residual 在 \(\alpha_0\neq0\) 時有 \(10^{-4}\sim10^{-3}\) 等級。

boundary flux balance diagnostic 顯示：

- 非零 mass residual 主要來自 surface term。
- 不是 flat open boundary flux 造成。

### 4.2 direct numerical flux 嘗試
...
### 4.3 pairwise single-edge common flux
...
## 5. 第四階段：sphere seam connectivity

### 5.1 核心做法

flat-square boundary 不再使用 `same_state`，而是用球面 embedding \((X,Y,Z)\) 對 boundary faces 進行配對：
...
## 6. 第五階段：edge-streamfunction surface flux

### 6.1 問題

direct surface flux 不保 constant state。  
因此改回 strong correction：
...
- `constant`：
  ...
  \]
- `element_checker` / `element_jump`：
  \[
  I_{\rm full}\approx0.
  \]
- `gaussian`：
  mass residual 接近 roundoff。
- edge-pair flux error：
  \[
...
## 7. 第六階段：local correction 與 global mass correction

### 7.1 local conservative correction 嘗試
...
### 7.2 local correction 的問題

對 \(q=1\)，local correction 的 `massDrift` 仍為 roundoff，但 `corrMax` 在 \(\alpha_0\neq0\) 時不是 roundoff。
...
- constant-state preservation：
  ...
  \]
- Gaussian / sphere modes mass drift：
  \[
  \texttt{massDrift}\approx0.
  \]
- discontinuous element checker 單步 mass drift：
  \[
  10^{-16}\sim10^{-17}.
  \]
...
## 8. 第七階段：Numba fast RHS

### 8.1 加速目標

最耗時部分：

1. 每次 RHS 的 face edge flux loop。
2. pairwise penalty assembly。
3. surface integral accumulation。
4. LSRK 五 stage 反覆呼叫 RHS。

### 8.2 加速策略

保持數學結構不變，只將 surface-related kernels Numba 化：

- edge-streamfunction normal flux kernel；
- pairwise strong penalty kernel；
- surface integral accumulation kernel；
- global mass residual kernel；
- global correction application kernel。
...
## 9. 第八階段：accuracy / convergence test
...
- CFL=1.0 與 CFL=0.5 結果幾乎相同。
- 因此 time error 不是主因。
- 長時間 \(L^2\) rate 約退化到二階上下。

### 9.5 Gaussian center 測試
...
1. seam / patch corner interaction；
2. volume-face flux consistency mismatch；
3. equal-area octahedral map 在 patch corner 附近的 regularity loss；
4. edge-streamfunction surface flux 與 volume flux 長時間相位誤差累積。

---

## 10. face-flux consistency diagnostic
...
### 10.4 endpoint diagnostic

Table1 face trace nodes 不含真正 endpoint，所以 exact endpoint mask 結果為 NaN。  
top error CSV 顯示最大誤差發生在：
...
因此最大 mismatch 來自 **near-corner trace nodes**，不是真正 endpoint。

### 10.5 corner_volavg 嘗試
...
- \(L^\infty\) 略有改善。
- \(L^2\) 與 convergence rate 沒有改善。
- \(nsub=16\) 長時間 \(L^2\) 仍約 \(1.3\times10^{-4}\)。
...
| local-side flux assembly | `pairErr` 在 \(\alpha_0\neq0\) 時為 \(O(10^{-1})\) |
...
| corner_volavg face-speed blending | 未改善長時間 \(L^2\) rate |
...
1. 建立 equal-area sphere-flat geometry cache。
2. 建立 SDG Table1 differentiation operators。
3. 建立 streamfunction：
   \[
   \psi=-R\,\Omega\cdot X.
   \]
4. 建立 area flux：
   ...
   F^x=-\psi_y,\qquad F^y=\psi_x.
   \]
5. 建立 projected inverse mass：
   \[
   P=V(A V^TWV)^{-1}(A V^T).
   \]
6. 用 \((X,Y,Z)\) 建立 sphere seam connectivity。
7. 建立 face edge derivative matrix。
8. 預先計算：
   \[
...
2. Surface:
   - pairwise common numerical flux；
   - penalty:
     \[
     p=f-\hat f;
     \]
...
### 14.2 volume-face flux mismatch
...
在 near-corner trace nodes 有 \(O(1)\) mismatch。  
此 mismatch 可能長時間累積成 phase / amplitude error。

### 14.3 equal-area map regularity

octahedral equal-area map 在 patch corners 附近可能有限 regularity。  
即使 solution 本身在 sphere 上 smooth，映射到 flat patch 後可能在 corner / seam 結構下產生低 regularity 表現。

### 14.4 global correction 對 accuracy 的影響

global correction magnitude 通常很小，但仍可能在長時間中對 phase / amplitude 有微弱累積效應。  
目前未觀察到它是主要問題，但需在報告中標示為限制。
...
### 15.2 中期：accuracy 改善方向

可嘗試：

1. **face-volume consistency correction**  
   建立使 volume flux trace 與 edge flux trace 更一致的 split form。

2. **corner regularization**  
   對 near-corner trace nodes 設計更幾何一致的 face speed regularization，而不是簡單 volavg。

3. **corner-excluding benchmark**  
   建立不接近 octahedral vertices 的 manufactured solution / local-in-time benchmark，區分 scheme order 與 mapping singularity effect。

4. **multi-patch smoothness analysis**  
   分析 equal-area map 在 octahedron edge / vertex 的 differentiability class。

5. **energy / \(L^2\) stability diagnostic**  
   目前只完整確認 mass 與 constant state，還需測 energy behavior。
...
1. 避免 \(A^{-1}\) pole ambiguity。
2. 保留 equal-area map。
3. seam pairing 使用 embedding coordinates。
4. pairwise flux 保 conservation。
5. edge-streamfunction face speed 保 free-stream。
6. Numba fast RHS 可做實驗。
7. 明確揭示 long-time accuracy 受 patch-corner regularity 限制。
